...
.PHONY: all
all: tut112-multi-gpu.tar.gz

SOURCES=$(shell gfind . -maxdepth 1 -mindepth 1 -not -path "./.*" -not -name "tut112-multi-gpu.tar.gz" -printf '%P\n' | sort -h)

tut112-multi-gpu.tar.gz: $(shell find . -not -name "tut112-multi-gpu.tar.gz")
	sed -i '1 i***Please check GitHub repo for latest version of slides: https://github.com/FZJ-JSC/tutorial-multi-gpu/ ***\n' README.md
	tar czf $@ --transform 's,^,ISC26-tut112-Multi-GPU/,' --exclude=".*" $(SOURCES)
	tar xf $@
	rm $@
	find ISC26-tut112-Multi-GPU/ -not -path './.*' -iname 'slides-*.pdf' -execdir rm slides.pdf \;
	tar czf $@ ISC26-tut112-Multi-GPU
	rm -rf ISC26-tut112-Multi-GPU
